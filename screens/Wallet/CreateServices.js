...
const { cameraRollPermission_P, createServices_P, info_W, nameOfService_P, category_W, price_W, description_W, describeService_P, photo_W, insertImage_P, create_W } = tokens.screens.wallet.createServices
  ...
  useEffect(() => {
    (async () => {
      if (Platform.OS !== 'web') {
        ...
        if (status !== 'granted') {
          alert(cameraRollPermissionPhrase);
        }
      }
    })();
  }, []);
        ...
        <View style={styles.formContainer}>
          ...
          <TouchableOpacity style={styles.formContent} onPress={() => pickImage()}>
            <View style={{ flexDirection: "row", justifyContent: "center", alignItems: "center" }}>
              <Icon name="drive-file-rename-outline" style={{ color: orange, fontSize: Dimensions.get("screen").height * 0.05, marginRight: 5 }}></Icon>
              <Text style={{ color: "#999" }}>{insertImagePhrase}</Text>
            </View>
          </TouchableOpacity>
        </View>
